#!/bin/bash
# This script is used to extract gene sequences from GenBank files and perform BLAST searches against a database of bacterial genomes for multiple BGCs.
...
path_to_reference="/Users/linyusheng/Grazing_Resist_Genes/BGCs/curated_bgc_genotype/${strain_name}/"
...
for bgc_dir in $bgc_dirs; do
    ...
    output_summary_table="$output_blast_results_dir/${strain_name}_${target_BGC}_targets_summary.txt"
    ...
    fasta_count=$(ls -1 $path_to_reference/$target_BGC/*.fasta 2>/dev/null | wc -l)
    if [ $fasta_count -eq 0 ]; then
        log_message "Error: No .fasta files found in directory: $path_to_reference/$target_BGC"
        exit 1
    fi

    log_message "Found $fasta_count .fasta files in directory: $path_to_reference/$target_BGC"

    # Concatenate all fasta files in the BGC directory to create a reference genotype fasta file
    if [ -f "$path_to_reference/$target_BGC/${strain_name}_${target_BGC}.fasta" ]; then
        rm -f "$path_to_reference/$target_BGC/${strain_name}_${target_BGC}.fasta"
        log_message "Removing already existed file: $path_to_reference/$target_BGC/${strain_name}_${target_BGC}.fasta"
        log_message "Creating reference genotype fasta file for $target_BGC..."
    else
        log_message "Creating reference genotype fasta file for $target_BGC..."
    fi
    cat $path_to_reference/$target_BGC/*.fasta > "$path_to_reference/$target_BGC/${strain_name}_${target_BGC}.fasta"
    output_ref_geno_fasta="${strain_name}_${target_BGC}.fasta"

    # Loop through all .ffn files in the database directory and run BLAST searches
